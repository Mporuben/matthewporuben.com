...
<script lang="ts" setup>
  ...
  const {data: categories} = await useAsyncData(async() => {
    const {categories} = await queryContent('blog', 'categories').findOne()
    return categories.map((category) => ({value: category.title, enabled: false}))
  })

  const fulltext = ref('')

  const selectedCategories = computed(() => categories.value.reduce((acc, el) => el.enabled ? acc.concat([el.value]) : acc, []))
